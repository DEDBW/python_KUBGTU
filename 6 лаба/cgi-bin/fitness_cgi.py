...
def display_table(table_name):
    ...
    print("<th>Редактировать</th><th>Удалить</th>")
    print("</tr>")
    for row in rows:
        record_id = row[0]
        ...
        print(f"<td><a href='/cgi-bin/edit_client.py?id={record_id}'>Редактировать</a></td>")
        print(f"<td><a href='delete_client.py?id={record_id}' onclick=\"return confirm('Вы уверены, что хотите удалить эту запись?')\">Удалить</a></td>") # Ссылка "Удалить" с подтверждением
        print("</tr>")
    print("</table>")
...
print("<br>")
print("<button onclick=\"location.href='../add_client_form.html'\" type='button'>Добавить клиента</button>")
print("</body></html>")